...
const ProductDetails = () => {
  ...
  const { id, name, price, description, rating, reviews, colors, moreImages , photos } =
    state;
  ...
  const handleAddToCart = () => {
    dispatch(addToCart({ id, name, price, description, reviews , photos }));
  };
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-wrap gap-[25px] ">
            {parsedColors.map((color , index) => (
              <ColorBox key={index} name={color.name} colors={[color]}  />
            ))}
          </div>
